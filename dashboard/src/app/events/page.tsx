...
export default function EventsPage() {
  ...
  useEffect(() => {
    if (!localStorage.getItem("larpchekr_jwt")) window.location.href = "/sign-in";
    void load();
  }, []);
    ...
    <main className="min-h-screen bg-[#f6f2e8] p-10 text-stone-950">
      <div className="mx-auto max-w-5xl">
        ...
        {!loading && events.length === 0 ? (
          <form onSubmit={create} className="mt-8 rounded-3xl border border-dashed border-stone-400 bg-white p-6">
            <h2 className="text-2xl font-bold">No events yet</h2>
            <p className="mt-1 text-stone-600">Create a fixture event for imports.</p>
            <div className="mt-4 flex gap-3">
              <input className="flex-1 rounded-xl border border-stone-300 px-4 py-3" value={name} onChange={(e) => setName(e.target.value)} />
